...
const CardItem2: React.FC<cardProps> = ({
  ...
  return (
    <div className="relative h-96 w-[16.5rem] overflow-hidden rounded-[0.6rem] text-white shadow-lg">
      ...
      <div className="absolute right-3 bottom-4 left-3 flex flex-col gap-1">
        <h3 className="text-lg leading-snug font-semibold">{title}</h3>
        <p className="text-text-400 line-clamp-3 text-sm leading-snug">{subtitle}</p>
      </div>
    </div>
  );
};
